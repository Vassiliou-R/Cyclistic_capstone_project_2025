...
```{r}
install.packages(c("tidyverse", "lubridate", "janitor", "skimr", "ggplot2", "scales", "ggmap", "leaflet", "viridis", "sf"))
library(tidyverse)
library(lubridate)
...
# Organize columns in the two datasets to match each other and be able to combine them

```{r}
# Rename the columns in dataset Divvy_Trips_2019_Q1 to match the columns names in dataset Divvy_Trips_2020_Q1
clean_2019 <- q1_2019 %>%
...
summary_stats <- trips_clean %>%
  group_by(member_casual, day_of_week) %>% # Group data by usertype and day_of_week
  summarise(
...
# VISUALIZATIONS
## USERTYPE COMPOSITION
# 1. Total usertypes (members/casuals)
...
# 2. Average ride duration in minutes by usertype
...
# 3. Total rides count by usertype
...
### C. MONTHLY TRENDS
# 4. Average ride duration by month by usertype
...
# 5. Total ride count by month by usertype
...
### D. DAY OF WEEK TRENDS
# 6. Average ride duration by day of week by usertype
...
# 7. Total rides count by day of week by usertype
...
### E. WEEKDAY VS WEEKEND TRENDS
# 8. Average ride duration by weekday - weekend by usertype
...
# 9. Total rides count by weekday - weekend by usertype
